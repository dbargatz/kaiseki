...
fn build(sdk_path: Option<&str>, target: &str) {
    ...
    builder = builder
        ...
        .header_contents("Virtualization.h", "#include<Virtualization/Virtualization.h>");
    ...
    bindings
        .write_to_file(out_dir.join("virtualization_bindings.rs"))
...
fn main() {
    ...
    if !target.contains("apple-darwin") {
        panic!("kaiseki-backend-virtualization requires the *-apple-darwin target (target is {})", target);
    }
